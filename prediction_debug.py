...
class Config:
    ...
    # Obj1, Obj2 프롬프트 (Obj2도 필요함)
    if not REVERSED :
        OBJ_1_POINTS = np.array([[820, 380], [820, 700]], dtype=np.float32)
        OBJ_1_LABELS = np.array([1, 1], dtype=np.int32)
        OBJ_2_POINTS = np.array([[820, 270], [820, 800], [820, 380], [820, 700]], dtype=np.float32)
    ...
    # 디버그 플래그
    DEBUG = True
    DEBUG_SHOW_MASK_FILTERING = True      # Depth threshold로 마스크 필터링 전후 비교
    DEBUG_SHOW_PLANE_FITTING = True       # 평면 피팅 결과 (inliers vs outliers)
    DEBUG_SHOW_ICP_ALIGNMENT = True       # ICP 정렬 전후 비교
    DEBUG_SHOW_MEASUREMENT_POINTS = False  # 측정점과 교차점 3D 시각화
...
def filter_points_by_mask(points_3d_cam, mask, K, D, W, H, depth_threshold=None, depth_range=None, update_mask=False):
    """
    ...
    Args:
        ...
        K, D: 카메라 파라미터
        W, H: 이미지 크기
        depth_threshold: Z값 중심값
        depth_range: depth_threshold ± depth_range 범위 내의 점만 선택 (None이면 threshold만 적용)
    ...
    if depth_threshold is not None:
        if depth_range is not None:
            # depth_threshold ± depth_range 범위 내의 점만 선택
...
# 디버그 시각화 함수들
# ========================================
def debug_show_mask_filtering(obj2_pts_before, obj2_pts_after, mask_before, mask_after, frame_idx):
    """Depth threshold 필터링 전후 비교"""
    if not Config.DEBUG or not Config.DEBUG_SHOW_MASK_FILTERING:
        return
    
    print(f"\n🔍 [DEBUG] Frame {frame_idx}: Mask Filtering")
    print(f"   Before: {len(obj2_pts_before)} points")
    print(f"   After: {len(obj2_pts_after)} points (removed {len(obj2_pts_before) - len(obj2_pts_after)})")
    
    # 3D 시각화
    geoms = []
    
    # Before (빨강)
    if len(obj2_pts_before) > 0:
        pcd_before = o3d.geometry.PointCloud()
        pcd_before.points = o3d.utility.Vector3dVector(obj2_pts_before.astype(np.float64))
        pcd_before.paint_uniform_color([1.0, 0.0, 0.0])  # 빨강
        geoms.append(pcd_before)
    
    # After (초록)
    if len(obj2_pts_after) > 0:
        pcd_after = o3d.geometry.PointCloud()
        pcd_after.points = o3d.utility.Vector3dVector(obj2_pts_after.astype(np.float64))
        pcd_after.paint_uniform_color([0.0, 1.0, 0.0])  # 초록
        geoms.append(pcd_after)
    
    geoms.append(o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0))
    show_geometries_with_backface(geoms, title=f"Frame {frame_idx}: Depth Filter (Red=Before, Green=After)")


def debug_show_plane_fitting(obj2_pts, normal, centroid, inlier_mask, frame_idx):
    """평면 피팅 결과 (inliers vs outliers)"""
    if not Config.DEBUG or not Config.DEBUG_SHOW_PLANE_FITTING:
        return
    
    if normal is None or len(obj2_pts) == 0:
        return
    
    inliers = obj2_pts[inlier_mask]
    outliers = obj2_pts[~inlier_mask]
    
    print(f"\n📐 [DEBUG] Frame {frame_idx}: Plane Fitting")
    ...
    if len(inliers) > 0:
        ...
        pcd_in.points = o3d.utility.Vector3dVector(inliers.astype(np.float64))
    ...
    if len(outliers) > 0:
        ...
        pcd_out.points = o3d.utility.Vector3dVector(outliers.astype(np.float64))
    ...
    arrow = o3d.geometry.TriangleMesh.create_arrow(
        ...
        cylinder_height=0.3, cone_height=0.1
    )
    # 법선 방향으로 회전
    ...
    arrow.paint_uniform_color([0, 0, 1])  # 파랑
    geoms.append(arrow)
    
    geoms.append(o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0))
    show_geometries_with_backface(geoms, title=f"Frame {frame_idx}: Plane Fit (Green=Inliers, Red=Outliers)")


def debug_show_icp_alignment(obj1_mesh, source_pts, target_pts, T_icp, frame_idx):
    """ICP 정렬 전후 비교"""
    if not Config.DEBUG or not Config.DEBUG_SHOW_ICP_ALIGNMENT:
        return
    
    print(f"\n🔌 [DEBUG] Frame {frame_idx}: ICP Alignment")
    
    geoms = []
    
    # Obj1 메쉬 추가 (반투명 주황색)
    if obj1_mesh is not None:
        mesh_copy = copy.deepcopy(obj1_mesh)
        mesh_copy.paint_uniform_color([1.0, 0.5, 0.0])  # 오렌지색
        geoms.append(mesh_copy)
        
        # 메쉬 와이어프레임
        wireframe = o3d.geometry.LineSet.create_from_triangle_mesh(mesh_copy)
        wireframe.paint_uniform_color([0.0, 0.0, 0.0])  # 검은색
        geoms.append(wireframe)
    
    # Source Before (빨강)
    pcd_src_before = o3d.geometry.PointCloud()
    pcd_src_before.points = o3d.utility.Vector3dVector(source_pts.astype(np.float64))
    pcd_src_before.paint_uniform_color([1.0, 0.0, 0.0])
    geoms.append(pcd_src_before)
    
    # Source After (초록)
    pcd_src_after = o3d.geometry.PointCloud()
    pcd_src_after.points = o3d.utility.Vector3dVector(source_pts.astype(np.float64))
    pcd_src_after.transform(T_icp)
    pcd_src_after.paint_uniform_color([0.0, 1.0, 0.0])
    geoms.append(pcd_src_after)
    ...
    geoms.append(o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0))
    show_geometries_with_backface(geoms, title=f"Frame {frame_idx}: ICP (Orange=Obj1, Red=Before, Green=After, Blue=Target)")


def debug_show_measurement_points(measure_pts, end_pts, slab_corners, frame_idx):
    """측정점과 교차점 3D 시각화"""
    if not Config.DEBUG or not Config.DEBUG_SHOW_MEASUREMENT_POINTS:
        return
    
    print(f"\n📏 [DEBUG] Frame {frame_idx}: Measurement Points")
    
    geoms = []
    
    # 측정 시작점들 (노랑 구)
    for i, pt in enumerate(measure_pts):
        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.05)
        sphere.translate(pt)
        sphere.paint_uniform_color([1.0, 1.0, 0.0])  # 노랑
        geoms.append(sphere)
    ...
    for i, pt in enumerate(end_pts):
        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.05)
        sphere.translate(pt)
        sphere.paint_uniform_color([1.0, 0.0, 0.0])  # 빨강
        geoms.append(sphere)
    
    # 측정 라인 (파랑)
    for i in range(len(measure_pts)):
        line_pts = np.array([measure_pts[i], end_pts[i]])
        line = o3d.geometry.LineSet()
        line.points = o3d.utility.Vector3dVector(line_pts)
        line.lines = o3d.utility.Vector2iVector([[0, 1]])
        line.paint_uniform_color([0.0, 0.0, 1.0])  # 파랑
        geoms.append(line)
    
    # 철판 코너들 (초록 구)
    if slab_corners is not None:
        for corner in slab_corners:
            sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.03)
            sphere.translate(corner)
            sphere.paint_uniform_color([0.0, 1.0, 0.0])  # 초록
            geoms.append(sphere)
    
    geoms.append(o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0))
    show_geometries_with_backface(geoms, title=f"Frame {frame_idx}: Measurements (Yellow=Start, Red=End)")
...
predictor.add_new_points_or_box(
    inference_state=inference_state, frame_idx=0, obj_id=obj_id_1, points=Config.OBJ_1_POINTS, labels=Config.OBJ_1_LABELS
)
predictor.add_new_points_or_box(
    inference_state=inference_state, frame_idx=0, obj_id=obj_id_2, points=Config.OBJ_2_POINTS, labels=Config.OBJ_2_LABELS
)

# ... (Propagation 및 파일 목록 로드 부분은 그대로 유지) ...
...
# ========================================
# 12. 메인 루프 (Matrix Direct Apply 수정)
# ========================================
...
for f_idx, fname in enumerate(frame_names):
    ...
    for oid in [obj_id_1, obj_id_2]:
        ...
        if mask is not None:
            ...
            # Erosion은 draw_mask_and_rbox 내부에서 이미 수행됨
            mask_eroded = apply_erosion(mask, Config.EROSION_KERNEL_SIZE, Config.EROSION_ITERATIONS) if Config.APPLY_EROSION else mask
            corners, _, _, _, _, _, _, _, _, _, _ = mask_to_rotated_box(mask_eroded)

            if corners is not None:
                if oid == obj_id_1: obj1_corners = corners
                elif oid == obj_id_2: obj2_rbox_corners = corners
    ...
    if obj1_corners is not None:
        ...
        try:
            res = least_squares(cost_function, initial_param, 
            ...
            # Obj2 LiDAR 점들 추출 (Depth Threshold 적용)
            if pcd_path and os.path.exists(pcd_path) and mask_obj2 is not None:
                ...
                # Before filtering (디버그용)
                obj2_pts_before = filter_points_by_mask(P_full_cam, mask_obj2, K_camera, D_dist, W, H)
                
                obj2_pts_cam, mask_obj2_updated = filter_points_by_mask(
                    P_full_cam, mask_obj2, K_camera, D_dist, W, H, 
                    depth_threshold=Config.DEPTH_TH, depth_range=Config.DEPTH_RANGE, update_mask=True
                )
                print(f"   🔍 Obj2 filtered: {len(obj2_pts_cam)} points (depth: {Config.DEPTH_TH}±{Config.DEPTH_RANGE}m)")
                
                # 디버그: Depth 필터링 전후 비교
                debug_show_mask_filtering(obj2_pts_before, obj2_pts_cam, mask_obj2, mask_obj2_updated, f_idx)
                
                # 업데이트된 마스크로 rbox corners 재추출 및 재시각화
                if mask_obj2_updated is not None:
                    mask_obj2 = mask_obj2_updated  # 마스크 업데이트
                    
                    # 기존 마스크 시각화 제거하고 업데이트된 마스크로 다시 그리기
                    draw_mask_and_rbox(
            ...
            if (obj2_rbox_corners is not None and 
                ...
                if normal_obj2 is not None:
                    ...
                    if synthetic_plane_cloud is not None and len(synthetic_plane_cloud) > 10:
                        ...
                        # C. ICP 수행 (Source: Obj1 Bottom -> Target: Obj2 Virtual Plane)
                        # Point-to-Plane 방식을 쓰면 평면 위에서 미끄러지는(Translation) 자유도는 
                        # 건드리지 않고, Z축 높이와 평면 기울기(Roll/Pitch)만 맞춰줍니다.
                        T_icp_final = refine_pose_icp_constrained(obj1_bottom_cloud, synthetic_plane_cloud, max_iteration=30)
                        print(f"   ✅ ICP Constrained Result:\n{T_icp_final}")
                        
                        # 디버그: ICP 정렬 전후 비교
                        debug_show_icp_alignment(base_mesh, obj1_bottom_cloud, synthetic_plane_cloud, T_icp_final, f_idx)
                        
                        # [중요] ICP 결과가 너무 크면(튀면) 무시 (안전장치)
                        delta_t = np.linalg.norm(T_icp_final[:3, 3])
                        if delta_t > 1.0: # 1m 이상 움직이려 하면 무시
            ...
            # ICP 시각화용 포인트 (가상 평면 점군을 시각화하고 싶으면 여기 할당)
            if synthetic_plane_cloud is not None:
                # 시각화를 위해 Open3D 형식으로 변환 (메모리 절약을 위해 다운샘플링 해도 됨)
                final_icp_points = synthetic_plane_cloud

            # ============================================================
            # 측정값 계산 (ICP 정합 후)
            # ============================================================
            len_top_mm = len_bot_mm = width_mm = 0.0
            
            if obj2_rbox_corners is not None and len(obj2_rbox_corners) == 4:
                # 최종 Pose 계산: estimated_param + ICP Transform 적용
                # T_icp_final를 estimated_param에 적용하여 최종 포즈 얻기
                
                # 1. estimated_param으로 초기 Box 생성
                temp_mesh, _ = get_3d_box_mesh(estimated_param, color=[1, 0, 0])
                temp_mesh.transform(T_icp_final)
                
                # 2. 변환된 Box의 8개 꼭짓점 추출
                final_vertices = np.asarray(temp_mesh.vertices) # (8, 3)
                
                # 3. Obj2(철판) 코너를 3D로 변환 (Raycasting)
                ...
                slab_corners_3d = []
                if normal_obj2 is not None and centroid_obj2 is not None:
                ...
                if len(slab_corners_3d) == 4:
                    ...
                    # ========== 길이 측정 (Length Measurements) ==========
                    # 마그넷 뒷면(Right Edge)의 두 측정점 정의 (로컬 좌표)
                    # 위쪽: Y=0.7m, 아래쪽: Y=2.25-0.7=1.55m, X=MAGNET_WIDTH (오른쪽 변)
                    
                    # 최종 변환된 꼭짓점 인덱스: 3=TR(top-right), 2=BR(bottom-right)
                    # Y축 보간으로 측정점 계산
                    top_left = final_vertices[0]  # TL (0.45, 0.0, z)
                    bottom_left = final_vertices[1]  # BL (0.45, 2.25, z)
                    
                    # 측정점 위치 (Y축 기준 0.7m, 1.55m)
                    t1 = 0.7 / MAGNET_LENGTH
                    t2 = (MAGNET_LENGTH - 0.7) / MAGNET_LENGTH
                    
                    measure_pt_top = top_left + t1 * (bottom_left - top_left)
                    measure_pt_bot = top_left + t2 * (bottom_left - top_left)
                    
                    # 측정 방향: 변환된 로컬 X축 방향 (1,0,0) -> 월드 좌표
                    R_icp = T_icp_final[:3, :3]
                    # estimated_param의 theta 각도
                    theta = estimated_param[2]
                    R_theta = np.array([
                    ...
                    measure_dir_2d /= np.linalg.norm(measure_dir_2d)
                    
                    # 철판 타겟 엣지 찾기 (측정 방향쪽 두 점)
                    slab_center = np.mean(slab_corners_3d[:, :2], axis=0)
                    ...
                    len_top_mm = ray_line_dist(measure_pt_top[:2], measure_dir_2d, edge_p1, edge_p2) * 1000
                    len_bot_mm = ray_line_dist(measure_pt_bot[:2], measure_dir_2d, edge_p1, edge_p2) * 1000
                    
                    # ========== 너비 측정 (Width Measurement) ==========
                    # 마그넷 위쪽 안쪽 코너 (TL: 0,0,0)에서 철판 윗변까지의 수직 거리
                    top_left = final_vertices[0]  # TL (0.0, 0.0, z)
                    
                    # 측정 방향: 로컬 Y축 음의 방향 (0,-1,0) -> 월드 좌표
                    width_dir_world = R_total @ np.array([0.0, -1.0, 0.0])
                    width_dir_2d = width_dir_world[:2]
                    width_dir_2d /= np.linalg.norm(width_dir_2d)
                    ...
                    width_dist = ray_line_dist(top_left[:2], width_dir_2d, edge_w1, edge_w2)
                    width_mm = width_dist * 1000
                    
                    # ========== 아래쪽 너비 측정 (Bottom Width Measurement) ==========
                    # 마그넷 아래쪽 안쪽 코너 (BL: 0,2.25,0)에서 철판 아랫변까지의 수직 거리
                    bottom_left = final_vertices[1]  # BL (0.0, 2.25, z)
                    
                    # 측정 방향: 로컬 Y축 양의 방향 (0,1,0) -> 월드 좌표 (아래쪽으로)
                    width_bottom_dir_world = R_total @ np.array([0.0, 1.0, 0.0])
                    width_bottom_dir_2d = width_bottom_dir_world[:2]
                    width_bottom_dir_2d /= np.linalg.norm(width_bottom_dir_2d)
                    ...
                    width_bottom_dist = ray_line_dist(bottom_left[:2], width_bottom_dir_2d, edge_wb1, edge_wb2)
                    width_bottom_mm = width_bottom_dist * 1000
                    
                    print(f"   📏 Length Top: {len_top_mm:.1f}mm, Bottom: {len_bot_mm:.1f}mm")
                    print(f"   📐 Width Top: {width_mm:.1f}mm, Bottom: {width_bottom_mm:.1f}mm")
                    
                    # 디버그: 측정 포인트 시각화 (compute end points first)
                    end_pt_top_3d_temp = measure_pt_top + dir_world * (len_top_mm / 1000.0)
                    end_pt_bot_3d_temp = measure_pt_bot + dir_world * (len_bot_mm / 1000.0)
                    end_pt_width_3d_temp = top_left + width_dir_world * (width_mm / 1000.0)
                    end_pt_width_bottom_3d_temp = bottom_left + width_bottom_dir_world * (width_bottom_mm / 1000.0)
                    
                    debug_show_measurement_points(
                        [measure_pt_top, measure_pt_bot, top_left, bottom_left],
                        [end_pt_top_3d_temp, end_pt_bot_3d_temp, end_pt_width_3d_temp, end_pt_width_bottom_3d_temp],
                        slab_corners_3d,
                        f_idx
                    )
                    ...
                    measurement_records.append({
                        'frame': f_idx,
                        'length_top_mm': len_top_mm,
                        'length_bottom_mm': len_bot_mm,
                        'width_top_mm': width_mm,
                        'width_bottom_mm': width_bottom_mm
                    })
                    
                    # ============================================================
                    # [수정됨] 측정 포인트 및 방향 시각화 (Point-to-Point 화살표)
                    # ============================================================
                    
                    # 1. 거리(m)를 이용해 3D 상의 끝점(교차점) 계산
                    # dir_world: 길이 측정 방향 (3D 단위 벡터)
                    # width_dir_world: 너비 측정 방향 (3D 단위 벡터)
                    ...
                    # 너비 측정 끝점
                    end_pt_width_3d = top_left + width_dir_world * (width_mm / 1000.0)
                    end_pt_width_bottom_3d = bottom_left + width_bottom_dir_world * (width_bottom_mm / 1000.0)
                    
                    # 2. 2D 투영 (시작점 4개 + 끝점 4개)
                    pts_to_project = np.array([
                        measure_pt_top, measure_pt_bot, top_left, bottom_left,                      # 시작점 (Indices: 0, 1, 2, 3)
                        end_pt_top_3d, end_pt_bot_3d, end_pt_width_3d, end_pt_width_bottom_3d      # 끝점 (Indices: 4, 5, 6, 7)
                    ])
                    
                    img_measure_pts, _ = cv2.projectPoints(pts_to_project, np.zeros(3), np.zeros(3), K_camera, D_dist)
                    ...
                    # 3. 그리기 (화살표가 시작점 -> 끝점으로 이어지도록 설정)
                    # xy=끝점(화살표 머리), xytext=시작점(화살표 꼬리)
                    
                    # (1) 길이 측정 - 위쪽 (노란색)
                    ax.plot(p_start_top[0], p_start_top[1], 'o', color='yellow', markersize=6, markeredgecolor='black')
                    ax.plot(p_end_top[0], p_end_top[1], 'x', color='yellow', markersize=6, markeredgecolor='black') # 끝점 표시
                    ...
                    ax.text(mid_top[0], mid_top[1] - 15, f'{len_top_mm:.0f}mm', 
                    ...
                    ax.text(mid_bot[0], mid_bot[1] + 20, f'{len_bot_mm:.0f}mm', 
                    ...
                    ax.text(mid_width[0] - 40, mid_width[1], f'{width_mm:.0f}mm', 
                    ...
                    ax.text(mid_width_bottom[0] - 40, mid_width_bottom[1], f'{width_bottom_mm:.0f}mm', 
                           color='lime', fontsize=9, weight='bold', ha='right',
                           bbox=dict(boxstyle='round,pad=0.2', facecolor='black', alpha=0.6))
           

            # 2D Reprojection (Transformed vertices)
            ...
            pose_text = (
                f"6DOF Pose:\n"
                f"Trans: ({tx:.2f}, {ty:.2f}, {tz:.2f})m\n"
                f"Rot: ({np.degrees(theta):.1f})°\n"
                f"Length: T={len_top_mm:.1f}mm B={len_bot_mm:.1f}mm\n"
                f"Width: T={width_mm:.1f}mm B={width_bottom_mm:.1f}mm"
            )
            ax.text(20, 40, pose_text, color='white', fontsize=10, bbox=dict(facecolor='black', alpha=0.5))
    ...
    plt.savefig(os.path.join(Config.OUTPUT_DIR, f"frame_{f_idx:05d}.jpg"), dpi=TARGET_DPI, bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    ...
    if Config.SHOW_O3D and pcd_path and os.path.exists(pcd_path):
        ...
        # 업데이트된 mask_obj2를 visualize_full_3d에 전달 (색칠 및 평면 자르기에 반영)
        visualize_full_3d(
            pcd_lidar=pcd, T_l2c=T_l2c, 
            mask_obj1=mask_obj1, mask_obj2=mask_obj2,  # 업데이트된 마스크 사용
            K=K_camera, dist_coeffs=D_dist, W=W, H=H, 
            max_depth=Config.MAX_DEPTH, 
            estimated_box=final_box_mesh,         # 변환된 Mesh 전달
            estimated_wireframe=final_wireframe,  # 변환된 Wireframe 전달
            obj2_rbox_corners=obj2_rbox_corners,  # 업데이트된 rbox corners
            icp_generated_points=final_icp_points, # 변환된 Points 전달
            target_model_points=P_target_icp if 'P_target_icp' in locals() else None,
            obj1_plane_mesh=obj1_plane_mesh if 'obj1_plane_mesh' in locals() else None
        )
...
if len(measurement_records) > 0:
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['frame', 'length_top_mm', 'length_bottom_mm', 'width_top_mm', 'width_bottom_mm']
